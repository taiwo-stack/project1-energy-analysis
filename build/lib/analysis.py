...
class Analyzer:
    ...
    def load_data(self, date_range: Optional[Tuple[datetime.date, datetime.date]] = None) -> pd.DataFrame:
        ...
        try:
            ...
            # Remove negative energy values (outliers)
            if 'energy_demand' in df.columns:
                initial_energy_count = len(df)
                negative_energy = df['energy_demand'] < 0
                negative_count = negative_energy.sum()
                
                if negative_count > 0:
                    df = df[~negative_energy].copy()
                    logger.info(f"Removed {negative_count} negative energy values ({initial_energy_count} -> {len(df)} records)")
            ...
            for col in temp_columns:
                if col in df.columns and len(df) > 0:
                    ...
                    temp_outlier_mask = (df[col] < min_temp) | (df[col] > max_temp)
                    temp_outlier_count = temp_outlier_mask.sum()
                    
                    if temp_outlier_count > 0:
                        df = df[~temp_outlier_mask | df[col].isna()].copy()  # Keep NaN values
                        logger.info(f"Removed {temp_outlier_count} temperature outliers in {col} outside [{min_temp}, {max_temp}]°F ({initial_temp_count} -> {len(df)} records)")
    ...
    def generate_data_quality_report(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        try:
            ...
            # Check negative energy values (outliers)
            if 'energy_demand' in df.columns:
                negative_energy = df['energy_demand'] < 0
                negative_count = negative_energy.sum()
                summary['outliers'] += negative_count
                
                if negative_count > 0:
                    issues.append(f"Negative energy values: {negative_count}")
