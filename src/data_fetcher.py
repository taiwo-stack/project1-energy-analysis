...
class DataFetcher:
    def __init__(self, config):  # Use your proper Config class
        self.config = config
    ...
    def fetch_noaa_data(self, station_id: str, start_date: str, end_date: str) -> Optional[Dict]:
        """Fetch NOAA weather data for a station."""
    ...
    def fetch_eia_data(self, region: str, start_date: str, end_date: str) -> Optional[Dict]:
        """Fetch EIA energy data for a region."""
        start_date, end_date = self._constrain_date_range(start_date, end_date)
    ...
    def _fetch_eia_alternative(self, region: str, start_date: str, end_date: str) -> Optional[Dict]:
        """Alternative EIA fetch method with simplified parameters."""
        start_date, end_date = self._constrain_date_range(start_date, end_date)
    ...
    def fetch_city_data(self, city, start_date: str, end_date: str) -> Tuple[Optional[Dict], Optional[Dict]]:
        """Fetch both weather and energy data for a city."""
        weather_data = None
        energy_data = None
    ...
    def fetch_historical_data(self, days: int = 90) -> List[Tuple[str, Optional[Dict], Optional[Dict]]]:
        """Fetch historical data for all cities over the specified number of days."""
        days = min(days, self.config.rate_limits['max_fetch_days'])
        
        # Stop buffer_days before current day to prevent availability issues
